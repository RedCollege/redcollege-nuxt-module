<template>
    <div class="progress-circle-base" :class="cn('relative size-20 text-base font-semibold', props.class)">
        <svg fill="none" class="size-full" stroke-width="2" viewBox="0 0 100 100">
            <circle v-if="currentPercent <= 90 && currentPercent >= 0" cx="50" cy="50" r="45"
                :stroke-width="circleStrokeWidth" stroke-dashoffset="0" stroke-linecap="round" stroke-linejoin="round"
                class="gauge-secondary-stroke opacity-100" />
            <circle cx="50" cy="50" r="45" :stroke-width="circleStrokeWidth" stroke-dashoffset="0"
                stroke-linecap="round" stroke-linejoin="round" class="gauge-primary-stroke opacity-100" />
        </svg>
        <span v-if="showPercentage" :data-current-value="currentPercent"
...
const props = withDefaults(defineProps<Props>(), {
    ...
    min: 0,
    value: 0,
    //gaugePrimaryColor: "rgb(179, 143, 0)",
...
const currentPercent = computed(() : number => ((props.value - props.min) / (props.max - props.min)) * 100);
const percentageInPx = computed(() => `${percentPx}px`);
const durationInSeconds = computed(() => `${props.duration}s`);

const gaugePrimaryColor = computed(() : string => {
    return currentPercent.value === 100 ? 'rgb(0, 163, 136)' : 'rgb(179, 143, 0)'
})
