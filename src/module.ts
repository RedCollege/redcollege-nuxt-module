...
export default defineNuxtModule<ModuleOptions>({
    ...
    async setup(options, nuxt) {
        ...
        nuxt.options.runtimeConfig.public.redcollege = defu(
            nuxt.options.runtimeConfig.public.redcollege,
            {
        ...
        try {
            ...
            console.warn(
                `Directory ${pagesDir} does not exist. Skipping page generation.`,
            );
            return;
        }
                ...
                items.forEach((item) => {
                    ...
                    if (!item.isDirectory() && item.name.endsWith(".vue")) {
                        const routeName = item.name
                            ...
                            .toLowerCase();
                        const fullRouteName =
                            `${basePath}/${routeName}`.substring(1);
                ...
                items.forEach((item) => {
                    ...
                    if (item.isDirectory()) {
                        ...
                        const parentPage = currentLevelPages.find(
                            (page) =>
                                page.name === item.name ||
                                page.path.endsWith(item.name),
                        );

                        const childBasePath = `${basePath}/${item.name}`;
                        const childPages = addPagesRecursively(
                            fullPath,
                            childBasePath,
                        );
        ...
        addLayout(
            resolver.resolve("runtime/layouts", "dashboard.vue"),
            "dashboard",
        );
        addLayout(
            resolver.resolve("runtime/layouts", "settings.vue"),
            "settings",
        );
        addLayout(
            resolver.resolve("runtime/layouts", "superadmin.vue"),
            "superadmin",
        );
        ...
        nuxt.options.alias["@redcollege/ui-nuxt-module"] =
            resolver.resolve(".");
